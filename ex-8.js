...
// Start coding here
// หา localtion ว่ามีกี่อัน
//เอา localtion เดี่ยวกัน + total ด้วยกัน

function findLocaltion(bills) {
  let allLocationAndTotal = {};
  for (let i = 0; i < bills.length; i++) {
    let location = bills[i].location;
    let total = bills[i].total;

    if (location in allLocationAndTotal) {
      allLocationAndTotal[location] += total;
    } else {
      allLocationAndTotal[location] = total;
    }
  }
  return allLocationAndTotal;
}

const totalPaidByLocation = findLocaltion(bills);
